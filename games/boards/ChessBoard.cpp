...
ChessBoard::ChessBoard(int flag){
	...
	if(flag == BOARD_INIT){
		...
		std::ifstream infile("files/initBoard.oc");
...
void ChessBoard::initBoardColor(){
	for(int i = 0; i < SIZE_BOARD_CHESS ; i++){
		for(int j = 0 ; j < SIZE_BOARD_CHESS ; j++){
...
void ChessBoard::movePieceTo(Position start_pos, Position end_pos){
...
Case ChessBoard::getCase(int row, int col){

	return cases[row][col];
}

pair<int, int>  ChessBoard::getCase(string color_piece, string name_piece){
	for( int i = 0 ; i < SIZE_BOARD_CHESS ; i++){
		for( int j = 0; j <SIZE_BOARD_CHESS ; j++){
...
void ChessBoard::setCase(int row, int col, Case c){
	this->cases[row][col] = c;
}

void ChessBoard::freeCase(int row, int col){
...
void ChessBoard::setNotification(string notif){
	this->notification_to_show = notif;
}
void ChessBoard::setNotification2(string notif){
	this->notification_to_show2 = notif;
}

string ChessBoard::getListPossibleMoves(string color_player){
	string ret = "";
	ChessRule rule;
	int i_i = 0;
...
vector< pair<Position,Position>  > ChessBoard::getListPossibleMovesVector(string color_player){
